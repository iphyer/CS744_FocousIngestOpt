...
lr_reducer = ReduceLROnPlateau(factor=np.sqrt(0.1), cooldown=0, patience=5, min_lr=1e-4)
early_stopper = EarlyStopping(min_delta=0.001, patience=10)
csv_logger = CSVLogger('newcutGPU.csv')
...
feature_map_train = np.load("model/newcut/feature_map_train.npy")
feature_map_test =  np.load("model/newcut/feature_map_test.npy")

print(feature_map_test.shape)
...
model_152_second_half.compile(loss='categorical_crossentropy',
              optimizer='adam',
              metrics=['accuracy'])

# train second half model using feature_map_train
model_152_second_half.fit(feature_map_train, Y_train,
...
model_152_second_half.save_weights("model/newcut_second_half_retrained.h5")
loss, acc = model_152_second_half.evaluate(feature_map_test, Y_test)

print(acc)

#plot_model(model_152_second_half, to_file='model.png',show_shapes = True)
